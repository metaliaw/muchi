# Referencia del Front anterior a la Migración de API

Este Documento conserva la Arquitectura y las Notas anteriores. No describe
el Front actual; consulta el README de la Raíz para ejecutarlo.
...
### La fuente muchi-api (opcional)

Además de scry.cl, Muchi puede sumar precios internacionales (TCGPlayer,
Cardmarket, Cardhoarder vía Scryfall) desde `muchi-api`, el backend propio
(`cangrejometralleta/muchi-api`, Go). Es opcional: sin las variables de
entorno de abajo, Muchi corre igual con scry.cl solo.

```bash
cp .env.example .env
```

Por defecto `.env` apunta a una instancia local — levanta `muchi-api` en el
repositorio vecino (`127.0.0.1:8081`) y corre Muchi normal. Para apuntar en
cambio a la instancia productiva en GCP, reemplaza ambas variables:

```bash
MUCHI_API_URL=https://muchi-serve-api-c2ce6c7oga-rj.a.run.app
MUCHI_API_TOKEN=<el del secret manager>
```

```bash
gcloud secrets versions access latest --secret=muchi-api-token --project=muchi-507614
```

En Streamlit Cloud, igual que `MUCHI_CONTACTO`, estas dos van en
*Settings → Secrets* — no en un `.env` commiteado.
...
- **🐟 Buscar** — una carta, todas las ofertas ordenadas por precio, la más barata
  ...
  Los Umbrales se configuran en `config/offers.defaults.yaml`.
...
Define `MUCHI_ENV=development` localmente y `MUCHI_ENV=production` en el
Despliegue. Es obligatorio al Arrancar la App, también para instalaciones
existentes. La Configuración se carga una vez: `config/stock.defaults.yaml`,
...
Los Umbrales de Ofertas viven en `config/offers.defaults.yaml`, con Overrides
por `offers.development.yaml` o `offers.production.yaml`. Las Variables son
`MUCHI_OFFERS_PRICE_GAP_RATIO`, `MUCHI_OFFERS_MINIMUM_OFFERS`,
`MUCHI_OFFERS_MAXIMUM_LOW_OFFERS`, `MUCHI_OFFERS_MAXIMUM_LOW_SHARE` y
`MUCHI_OFFERS_STOCK_CHECK_LIMIT`. Se validan al Arrancar y se inyectan en
las funciones de Ofertas. Los Defaults conservan el comportamiento anterior.
